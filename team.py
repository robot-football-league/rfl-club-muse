"""Muse Spark FC — Muse Spark 1.2 by Meta. Deterministic wrapper v4 — zero-LLM fast path."""
import math
...
class Wrapper:
    ...
    def decide(self, obs):
        try:
            ...
            # ball not seen -> search deterministically, no LLM
            if ball is None:
                # go to last known or center
                lb = self._last_ball
                # if near last ball, sweep center
                if _dist(sxy, lb) < 1.0:
            ...
            # never call LLM on exception, return safe walk
            try:
                det=obs.get("detections",{})
                ball=det.get("ball")
